...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<main${ssrRenderAttrs(mergeProps({ class: "thermal-ticket-page min-h-screen bg-slate-100 p-4 text-slate-950 print:min-h-0 print:bg-white print:p-0" }, _attrs))}><section class="thermal-ticket mx-auto max-w-[300px] bg-white p-4 font-mono shadow print:shadow-none"><h1 class="text-center text-lg font-black">Associação de Santana</h1><div class="text-center font-black">${ssrInterpolate(__props.pedido.ponto_bar)}</div><div class="text-center text-xs font-bold">Operador: ${ssrInterpolate(operador.value)}</div><div class="ticket-token my-4 border-y border-dashed border-slate-400 py-4 text-center"><div class="text-xs uppercase">Número da senha</div><div class="ticket-number text-5xl font-black">#${ssrInterpolate(__props.pedido.numero_senha || __props.pedido.id)}</div></div><div class="ticket-items space-y-2 text-lg font-black"><!--[-->`);
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/Pos/TalaoSenha.vue");
